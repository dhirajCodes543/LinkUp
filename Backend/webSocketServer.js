    ...
    ws.on('message', (msg) => {
      ...
      if (data.type === 'join' && Array.isArray(data.interests)) {
        ...
        if (matchIndex !== -1) {
          ...
          const payload = JSON.stringify({ type: 'room_assigned', roomId });
          
          // Send to both clients if still connected
          if (matchedClient.ws.readyState === 1) { // WebSocket.OPEN = 1
            matchedClient.ws.send(payload);
          }
          if (ws.readyState === 1) {
            ws.send(payload);
          }
          
          console.log(`Paired clients in room: ${roomId}`);
          ...
          waitingClients.push({ ws, interests: newInterests, timeout });
